...
#define ABRT_JOURNAL_XORG_MAX_READ_LINES (1024 * 1024)
...
abrt_xorg_process_list_of_crashes(GList *crashes, const char *dump_location, int flags)
{
    ...
    GList *list;
    int index = 0;
    for (list = crashes; list != NULL; list = list->next, ++index)
    {
        xorg_crash_info_create_dump_dir(list->data, dump_location, (flags & ABRT_XORG_WORLD_READABLE), index);
...
static GList *abrt_journal_extract_xorg_crashes(abrt_journal_t *journal)
{
    ...
    while (++lines_info_count < ABRT_JOURNAL_XORG_MAX_READ_LINES
...
static void watch_journald(abrt_journal_t *journal, const char *dump_location, int flags)
{
    ...
    xorg_strings = g_list_prepend(xorg_strings, (gpointer)"Backtrace:");
...
int main(int argc, char *argv[])
{
    ...
    /* SYSLOG_IDENTIFIER=gdm-x-session works for Fedora 22+ */
    xorg_journal_filter[0] = (env_journal_filter ? env_journal_filter : "SYSLOG_IDENTIFIER=gdm-x-session");
